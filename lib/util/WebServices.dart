...
class WSAuthUser {
  ...
  static Future<WSAuthUser> get() async{
    ...
    if (boot != null) {
      ...
      Uri userUri = Uri.parse(boot['config']['links']['authentication/me'] + "?access_token=${WSBoot.token}");
      ...
      res.listen((event) {
        ...
        _completer.complete(WSAuthUser(user['email'], user['id'], user['name']));
      });
    }else _completer.complete(null);
...
class WSBoot {
  ...
  static Future<Map<String, dynamic>> get bootConf async {
    Completer<Map<String, dynamic>> _completer = new Completer<Map<String, dynamic>>();
    if (_conf == null || DateTime.now().millisecondsSinceEpoch >= _confExpiry) {
      ...
      if (!sp.containsKey("boot")) _completer.complete(null);
      else {
        HttpClient client = HttpClient();

        String bootUrl = sp.getString("boot");
        String params = bootUrl.substring(bootUrl.indexOf('?'));
        Uri actualUrl = Uri.parse(bootUrl.substring(0, bootUrl.indexOf('?')) + '/android/v3/1405/' + params); //TODO: iOS specific path when using iOS?
    ...
    }else _completer.complete(_conf);
    return _completer.future;
  }
}