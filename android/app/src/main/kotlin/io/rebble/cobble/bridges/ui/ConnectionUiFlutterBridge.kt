...
class ConnectionUiFlutterBridge
    @Inject
    constructor(
        ...
        @TargetApi(Build.VERSION_CODES.O)
        private fun processCompanionDeviceResult(
            resultCode: Int,
            data: Intent?
        ) {
        ...
        private fun openConnectionToWatch(macAddress: String) {
            pairCallbacks.onWatchPairComplete(
                Pigeons.StringWrapper.Builder().setValue(macAddress).build()
            ) {}
