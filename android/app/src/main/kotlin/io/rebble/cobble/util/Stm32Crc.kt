...
private fun UInt.calcCrc(word: UInt): UInt {
    var crc = this xor word
    crc = (crc shl 8) xor crcTable[((crc shr 24) and 0xFFU).toInt()]
    crc = (crc shl 8) xor crcTable[((crc shr 24) and 0xFFU).toInt()]
    crc = (crc shl 8) xor crcTable[((crc shr 24) and 0xFFU).toInt()]
    crc = (crc shl 8) xor crcTable[((crc shr 24) and 0xFFU).toInt()]
    return crc
...
fun BufferedSource.stm32Crc(): Long {
    ...
    fun processBuffer() {
        if (rem == 4) {
            val word = (buffer[0].toUInt() and 0xFFU) or
                    ((buffer[1].toUInt() and 0xFFU) shl 8) or
                    ((buffer[2].toUInt() and 0xFFU) shl 16) or
                    ((buffer[3].toUInt() and 0xFFU) shl 24)
            crc = crc.calcCrc(word)
            rem = 0
        }
    }
    ...
    if (rem > 0) {
        val word = when (rem) {
            3 -> (buffer[2].toUInt() and 0xFFU) or
                    ((buffer[1].toUInt() and 0xFFU) shl 8) or
                    ((buffer[0].toUInt() and 0xFFU) shl 16)
            2 -> (buffer[1].toUInt() and 0xFFU) or
                    ((buffer[0].toUInt() and 0xFFU) shl 8)
            else -> buffer[0].toUInt() and 0xFFU
        }
        crc = crc.calcCrc(word)
    }
    return crc.toLong()
