    ...
    override fun processCallNotification(sbn: StatusBarNotification): CallNotification? {
        val acceptAction =
            sbn.notification.actions?.firstOrNull {
                it.title.toString().contains("Answer", true)
            }
        val declineAction =
            sbn.notification.actions?.firstOrNull {
                it.title.toString().contains("Decline", true)
            }
        val hangUpAction =
            sbn.notification.actions?.firstOrNull {
                it.title.toString().contains("Hang up", true)
            }

        if (acceptAction != null && declineAction != null) {
            val contactName =
                sbn.notification.extras.getCharSequence("android.title")?.toString()?.let {
                    "WhatsApp\n$it"
                }
