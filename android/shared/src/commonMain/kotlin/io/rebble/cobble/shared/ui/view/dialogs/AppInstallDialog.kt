...
@Composable
fun AppInstallDialog(uri: String, onDismissRequest: () -> Unit) {
    ...
    AlertDialog(
            ...
            title = {
                when (appInstallState) {
                    ...
                    is AppInstallDialogViewModel.AppInstallState.Success -> Text("Successfully Installed")
                }
            },
            text = {
                when (appInstallState) {
                    null -> Text(
                            buildAnnotatedString {
                                ...
                                withStyle(style = SpanStyle(fontWeight = FontWeight.Bold)) {
                                    append(viewModel.app.info.longName.ifBlank { viewModel.app.info.shortName })
                                }
                                append(" by ")
                                withStyle(style = SpanStyle(fontWeight = FontWeight.Bold)) {
                                    append(viewModel.app.info.companyName.ifBlank { "<Not Specified>" })
                                }
            ...
            confirmButton = { if (appInstallState == null) {
                TextButton(
            ...
            dismissButton = { if (appInstallState == null) {
                TextButton(
                        content = { Text("Cancel") },
                        onClick = { onDismissRequest() }
                )
