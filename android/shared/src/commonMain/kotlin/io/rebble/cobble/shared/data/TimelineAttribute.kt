...
data class TimelineAttribute(
    ...
    companion object {
        fun fromProtocolAttribute(attr: TimelineItem.Attribute): TimelineAttribute {
            return when (val id = attr.attributeId.get().toInt()) {
                ...
                in 4..7, 13, 14 -> {
                    TimelineAttribute(
                        id,
                        uint32 = DataBuffer(attr.content.get()).getULong().toLong()
                    )
                }
        ...
        fun tinyIcon(icon: TimelineIcon) =
            TimelineAttribute(
                4,
                uint32 = icon.protocolValue.toLong() or 0x80000000
            )

        fun smallIcon(icon: TimelineIcon) =
            TimelineAttribute(
                5,
                uint32 = icon.protocolValue.toLong() or 0x80000000
            )

        fun largeIcon(icon: TimelineIcon) =
            TimelineAttribute(
                6,
                uint32 = icon.protocolValue.toLong() or 0x80000000
            )
        ...
        fun cannedResponse(responses: List<String>) =
            TimelineAttribute(
                8,
        ...
        fun shortTitle(shortTitle: String) =
            TimelineAttribute(
                9,
                string = shortTitle,
                maxLength = 64
        ...
        fun sportsGameState(state: TimelineGameState) =
            TimelineAttribute(
                23,
                uint8 = state.protocolValue
            )

        fun broadcaster(name: String) = TimelineAttribute(24, string = name)

        fun headings(headings: List<String>) =
            TimelineAttribute(
                25,
                listOfString = headings,
                maxLength = 128
            )

        fun paragraphs(paragraphs: List<String>) =
            TimelineAttribute(
                26,
                listOfString = paragraphs,
                maxLength = 1024
            )
        ...
        fun displayRecurring(recurring: Boolean) =
            TimelineAttribute(
                31,
                uint8 = if (recurring) 1 else 0
            )

        fun shortSubtitle(shortSubtitle: String) =
            TimelineAttribute(
                36,
        ...
        fun displayTime(displayTime: Boolean) =
            TimelineAttribute(
                38,
                uint8 = if (displayTime) 1 else 0
            )

        fun subtitleTemplateString(templateString: String) =
            TimelineAttribute(
                47,
