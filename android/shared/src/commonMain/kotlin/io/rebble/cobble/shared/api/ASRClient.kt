...
class ASRClient(
    ...
    fun createStreamingSession(block: suspend StreamingSessionScope.() -> Unit) = flow {
        val channel = Channel<ByteArray>(capacity = Channel.BUFFERED)
        val session = StreamingSessionScope(channel)
        val request = client.post("$baseUrl/NmspServlet/") {
            headers {
    ...
    class StreamingSessionScope(private val channel: Channel<ByteArray>) {
        suspend fun writeFrames(frameData: ByteArray) {
            writePart(frameData, "ConcludingAudioParameter", "AUDIO_INFO", "audio/x-speex-with-header-byte")
        }

        private suspend fun writePart(data: ByteArray, name: String, paramName: String, contentType: String) {
            val part = """
                --boundary
