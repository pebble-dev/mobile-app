...
class BlueLEDriver(
    ...
    override fun startSingleWatchConnection(device: PebbleDevice): Flow<SingleConnectionStatus> {
        ...
        return flow {
            ...
            check(
                gattServer.state.value == NordicGattServer.State.OPEN
            ...
            try {
                ...
                try {
                    withTimeout(20000) {
                        val result =
                            PPoGLinkStateManager.getState(device.address).first {
                                it != PPoGLinkState.ReadyForSession
                            }
                ...
                val rxJob =
                    gattServer.rxFlowFor(device.address)?.onEach {
                        rxStream.write(it)
