...
+ (instancetype)makeWithUuid:(NSString *)uuid
  ...
  AppReorderRequest* pigeonResult = [[AppReorderRequest alloc] init];
  pigeonResult.uuid = uuid;
  pigeonResult.newPosition = newPosition;
  return pigeonResult;
}
+ (AppReorderRequest *)fromMap:(NSDictionary *)dict {
  AppReorderRequest *pigeonResult = [[AppReorderRequest alloc] init];
  ...
  pigeonResult.newPosition = GetNullableObject(dict, @"newPosition");
  NSAssert(pigeonResult.newPosition != nil, @"");
  return pigeonResult;
}
- (NSDictionary *)toMap {
  return [NSDictionary dictionaryWithObjectsAndKeys:(self.uuid ? self.uuid : [NSNull null]), @"uuid", (self.newPosition ? self.newPosition : [NSNull null]), @"newPosition", nil];
}
@end
...
- (void)deleteCalendarPinsFromWatchWithCompletion:(void(^)(NSError *_Nullable))completion {
  FlutterBasicMessageChannel *channel =
    [FlutterBasicMessageChannel
...
- (void)handleTimelineActionActionTrigger:(nullable ActionTrigger *)arg_actionTrigger completion:(void(^)(ActionResponsePigeon *, NSError *_Nullable))completion {
  FlutterBasicMessageChannel *channel =
    [FlutterBasicMessageChannel
...
- (nullable id)readValueOfType:(UInt8)type 
{
  switch (type) {
    case 128:     
      return [AppReorderRequest fromMap:[self readValue]];
...
- (void)writeValue:(id)value 
{
  if ([value isKindOfClass:[AppReorderRequest class]]) {
    [self writeByte:128];
    [self writeValue:[value toMap]];
  } else 
...
- (void)beginAppOrderChangeAppReorderRequest:(nullable AppReorderRequest *)arg_appReorderRequest completion:(void(^)(NSError *_Nullable))completion {
  FlutterBasicMessageChannel *channel =
    [FlutterBasicMessageChannel
      messageChannelWithName:@"dev.flutter.pigeon.BackgroundAppInstallCallbacks.beginAppOrderChange"
      binaryMessenger:self.binaryMessenger
      codec:BackgroundAppInstallCallbacksGetCodec()];
  [channel sendMessage:@[arg_appReorderRequest] reply:^(id reply) {
    completion(nil);
  }];
}
@end
...
- (void)handleNotificationNotification:(nullable NotificationPigeon *)arg_notification completion:(void(^)(TimelinePinPigeon *, NSError *_Nullable))completion {
  FlutterBasicMessageChannel *channel =
    [FlutterBasicMessageChannel
  ...
  }
  {
    FlutterBasicMessageChannel *channel =
      [FlutterBasicMessageChannel
        ...
        codec:CalendarControlGetCodec()];
    if (api) {
      NSCAssert([api respondsToSelector:@selector(deleteCalendarPinsFromWatchWithError:)], @"CalendarControl api (%@) doesn't respond to @selector(deleteCalendarPinsFromWatchWithError:)", api);
      [channel setMessageHandler:^(id _Nullable message, FlutterReply callback) {
        FlutterError *error;
        [api deleteCalendarPinsFromWatchWithError:&error];
        callback(wrapResult(nil, error));
      }];
    }
    else {
  ...
  }
  {
    FlutterBasicMessageChannel *channel =
      [FlutterBasicMessageChannel
        ...
        codec:AppInstallControlGetCodec()];
    if (api) {
      NSCAssert([api respondsToSelector:@selector(beginAppOrderChangeAppReorderRequest:completion:)], @"AppInstallControl api (%@) doesn't respond to @selector(beginAppOrderChangeAppReorderRequest:completion:)", api);
      [channel setMessageHandler:^(id _Nullable message, FlutterReply callback) {
        NSArray *args = message;
        AppReorderRequest *arg_appReorderRequest = args[0];
        [api beginAppOrderChangeAppReorderRequest:arg_appReorderRequest completion:^(NumberWrapper *_Nullable output, FlutterError *_Nullable error) {
          callback(wrapResult(output, error));
        }];
      }];
    }
